...
const Navbar = ({ activePage, setActivePage, handleNavigateToCleaning }) => {
  ...
  return (
    ...
    <header className="w-full p-4 font-inter absolute top-0 z-50 px-20 py-5">
      {/* The navbar itself with the translucent, glowing effect */}
      <div className="group container mx-auto flex items-center justify-between p-3 bg-black/30 backdrop-blur-md rounded-4xl border-0.1 border-white shadow-[0_0_25px_rgba(255,255,125,0.3)] relative">
          ...
          <div className="relative">
            ...
            {isProductDropdownOpen && (
              <div 
                onMouseEnter={() => setIsProductDropdownOpen(true)}
                onMouseLeave={() => setIsProductDropdownOpen(false)}
                className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-48 bg-black/60 backdrop-blur-lg border border-white/10 rounded-lg shadow-lg z-50"
              >
                <button
